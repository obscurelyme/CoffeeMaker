...
Cursor::Cursor(const std::string& filePath) : _cursor(nullptr) {
  ...
  if (surface == nullptr) {
    ...
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "IMG_Load returned NULLPTR",
                             "Could not load cursor surface", NULL);
    exit(1);
  }
  SDL_SetColorKey(surface, SDL_TRUE,
                  SDL_MapRGB(surface->format, CoffeeMaker::Texture::COLOR_KEY.r,
