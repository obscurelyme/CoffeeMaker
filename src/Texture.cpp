...
SDL_Texture *CoffeeMaker::createRectTextureFromSurface(int height, int width,
  ...
  if (surface == NULL) {
    CoffeeMaker::MessageBox::ShowMessageBoxAndQuit("SDL_CreateRGBSurface Error",
                                                   "Could not create surface");
  }

  SDL_Texture *texture =
      SDL_CreateTextureFromSurface(Renderer::Instance(), surface);
  if (texture == NULL) {
    CoffeeMaker::MessageBox::ShowMessageBoxAndQuit(
        "SDL_CreateTextureFromSurface Error", "Could not create textgure");
  }
...
void Texture::SetTextureDirectory() {
  Texture::_textureDirectory =
      fmt::format("{}/{}", CoffeeMaker::Utilities::AssetsDirectory(), "images");
}
...
void Texture::LoadFromFile(const std::string &filePath) {
  ...
  if (surface == nullptr) {
    std::string msg =
        fmt::format("Could not load surface at filepath {}", filePath);
    CM_LOGGER_ERROR(msg);
    CoffeeMaker::MessageBox::ShowMessageBoxAndQuit("Error loading texture",
                                                   msg);
    return;
  }
  ...
  if (_useColorKey) {
    SDL_SetColorKey(
        surface, SDL_TRUE,
        SDL_MapRGB(surface->format, COLOR_KEY.r, COLOR_KEY.g, COLOR_KEY.b));
  }
  _texture =
      SDL_CreateTextureFromSurface(CoffeeMaker::Renderer::Instance(), surface);
  _height = surface->h;
  _width = surface->w;
  SDL_FreeSurface(surface);
}
...
void Texture::Render(int top, int left) {
  ...
  if (_texture == nullptr) {
    SDL_SetRenderDrawColor(CoffeeMaker::Renderer::Instance(), _color.r,
                           _color.g, _color.b, _color.a);
    SDL_RenderFillRect(CoffeeMaker::Renderer::Instance(), &renderQuad);
    return;
  }

  SDL_RenderCopy(CoffeeMaker::Renderer::Instance(), _texture, NULL,
                 &renderQuad);
}

void Texture::Render(float top, float left) {
  SDL_FRect renderQuad = {
      .x = left, .y = top, .w = (float)_width, .h = (float)_height};
  ...
  SDL_RenderCopyF(CoffeeMaker::Renderer::Instance(), _texture, NULL,
                  &renderQuad);
}
...
void Texture::Render(const SDL_Rect &clip, const SDL_Rect &renderRect) {
  ...
  SDL_RenderCopy(CoffeeMaker::Renderer::Instance(), _texture, &clip,
                 &renderRect);
}
...
void Texture::Render(const SDL_Rect &clip, const SDL_Rect &renderRect,
  ...
  if (_texture == nullptr) {
    CoffeeMaker::MessageBox::ShowMessageBoxAndQuit(
        "Error rendering texture", "Cannot render texture with nullptr");
    return;
  }
  SDL_RendererFlip flip = SDL_FLIP_NONE;
  SDL_RenderCopyEx(CoffeeMaker::Renderer::Instance(), _texture, &clip,
                   &renderRect, rotation, NULL, flip);
}
...
void Texture::Render(const SDL_Rect &renderRect, double rotation) {
  ...
  SDL_RendererFlip flip = SDL_FLIP_NONE;
  SDL_RenderCopyEx(CoffeeMaker::Renderer::Instance(), _texture, NULL,
                   &renderRect, rotation, NULL, flip);
}
