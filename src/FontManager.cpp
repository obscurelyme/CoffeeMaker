...
void FontManager::Init() {
  if (TTF_Init() == -1)
  {
    ...
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Font Manager: Init Failed", "Failed to intialize the FontManager, program will terminate.", nullptr);
    exit(1);
  }
}
...
{
  std::string fontFilePath = fmt::format("{}{}", CoffeeMaker::Utilities::AssetsDirectory(), fmt::format("/fonts/{}.ttf", fontName));
  ...
  if (font == nullptr) {
    ...
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Font Manager: LoadFont Failed", "The Font Mananger failed to load a font, program will terminate.", nullptr);
    exit(1);
  }
